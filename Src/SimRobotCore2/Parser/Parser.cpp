...
bool Parser::getColor(const char* key, bool required, float* color)
{
  ...
  if(*strColor == '#')
  {
    ...
    for(;;)
    {
      ...
      {
        location.column += strColor - strValue->c_str();
  ...
  else if(strncmp(strColor, "rgb(", 4) == 0)
  {
    // css style rgb color (rgb(r,g,b) with r,g,b\in[0..255]\cup[0%,..,100%])
    strColor += 4;
    for(int i = 0;; ++i)
    {
      while(isspace(*strColor))
        ++strColor;
      color[i] = strtof(strColor, const_cast<char**>(&strColor));
      if(*strColor == '%')
      {
        ++strColor;
        color[i] *= 0.01f;
  ...
  else if(strncmp(strColor, "rgba(", 5) == 0)
  {
